...
_total_files = os.listdir(_destination_path)
_total_len = int(len(_total_files))
_confirm_prompt = (input(f"Do you want to continue to {'COPY' if _mode=='c' else 'MOVE'} - {_total_len} files from {_destination_path} to {_source_path} (y/n) : ")).lower()
